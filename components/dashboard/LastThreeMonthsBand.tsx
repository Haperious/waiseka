...
export default function LastThreeMonthsBand({ months, movers, loading, formatAmount }: LastThreeMonthsBandProps) {
  ...
  return (
    ...
    >
        ...
        {loading ? (
          ...
          <div>
            ...
            {current && (
              ...
              >
                {current.label} is at a {current.savingsRate}% savings rate ({formatAmount(current.savings)} to savings
                on {formatAmount(current.income)} in)
                {prev
                  ? `, ${current.savingsRate >= prev.savingsRate ? "up" : "down"} from ${prev.savingsRate}% in ${prev.label}.`
                  : "."}
              </p>
            )}
          </div>
      ...
      {insufficientData && (
        <div
          style={{ padding: "0 20px 16px", fontSize: "0.76rem", lineHeight: 1.6, color: "var(--color-text-secondary)" }}
        >
          ...
          <div key={m.key} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
            <div style={{ display: "flex", alignItems: "flex-end", gap: 3, height: 108 }}>
              <Bar
                value={m.income}
                max={maxValue}
                color={loading ? "var(--color-elevated)" : "var(--color-income)"}
                animated={animated || loading}
              />
              <Bar
                value={m.expenses}
                max={maxValue}
                color={loading ? "var(--color-elevated)" : "var(--color-expense)"}
                animated={animated || loading}
              />
              <Bar
                value={m.savings}
                max={maxValue}
                color={loading ? "var(--color-elevated)" : "var(--color-savings)"}
                animated={animated || loading}
              />
            </div>
            <span
...
function MoverRow({
  ...
  const barVerdictColor =
    delta < 0 ? "var(--color-accent)" : delta > 5 ? "var(--color-expense)" : "var(--color-warning)"

  const figureColor =
    Math.abs(delta) < 2 ? "var(--color-text-secondary)" : delta < 0 ? "var(--color-income)" : "var(--color-expense)"
...
function MoversColumnSkeleton() {
  return (
    <div>
      <div
        style={{ height: 9, width: "70%", borderRadius: 4, backgroundColor: "var(--color-elevated)", marginBottom: 12 }}
      />
          ...
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ display: "flex", alignItems: "flex-end", gap: 2, height: 20, flexShrink: 0 }}>
              {[40, 60, 80].map((h, j) => (
                <div
                  key={j}
                  style={{ width: 6, height: `${h}%`, borderRadius: 2, backgroundColor: "var(--color-elevated)" }}
                />
              ))}
            </div>
            <div
