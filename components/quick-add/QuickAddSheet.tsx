...
export function ChipScroller({
  ...
  if (items.length === 0) {
    return emptyLabel ? (
      <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>{emptyLabel}</p>
    ) : null
  }
  return (
...
function VoiceMode({ onDone }: { onDone: () => void }) {
  ...
  const {
    transcript, isListening, isSupported, startListening, stopListening, clearTranscript,
  } = useSpeechToText()
  ...
  return (
    <div className="flex flex-col items-center gap-4 py-2">
      ...
      {noMatch && (
        <div className="w-full flex items-center justify-between rounded-lg px-3 py-2" style={{ backgroundColor: 'var(--color-expense-bg)' }}>
          <p className="text-xs" style={{ color: 'var(--color-expense)' }}>{t('quickAdd.voice.notHeard')}</p>
          <button onClick={handleRetry} className="text-xs font-semibold underline" style={{ color: 'var(--color-expense)' }}>
      ...
      {parsed && (
        ...
        >
          ...
          {parsed.description && (
            <p className="text-sm italic" style={{ color: 'var(--color-text-secondary)' }}>&ldquo;{parsed.description}&rdquo;</p>
          )}
          <div className="flex flex-wrap gap-1.5">
            {parsed.type && <Badge variant={parsed.type === 'expense' ? 'danger' : parsed.type === 'savings' ? 'savings' : 'success'}>{parsed.type}</Badge>}
  ...
  return (
    <div className="flex flex-col gap-4 py-2">
      ...
      >
        ...
        <p className="text-sm font-medium" style={{ color: 'var(--color-text-secondary)' }}>{t('quickAdd.receipt.title')}</p>
        <p className="text-xs text-center px-6" style={{ color: 'var(--color-text-muted)' }}>{t('quickAdd.receipt.subtitle')}</p>
      </div>
      ...
      <div className="rounded-xl p-4 space-y-2" style={{ backgroundColor: 'var(--color-card)' }}>
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="tabular-nums" style={{ color: 'var(--color-text-muted)' }}>—</span>
        </div>
      </div>
